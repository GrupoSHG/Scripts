...
// ============================================================
function TeamTab() {
  const isLoading = !D3.VENDEDORES || D3.VENDEDORES.every(v => !v.real || v.real.facturado === 0);
  ...
  return (
    <>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr 1fr', gap:10}}>
        <div className="card">
          <div className="card-head"><div className="card-title">Costos del depto. comercial</div><div className="card-sub">Mensual</div></div>
          <CostosTable />
        </div>
        <div className="card">
          <div className="card-head"><div className="card-title">Ahorro Gerencia Comercial</div><span className="badge green">Activo</span></div>
          <AhorroGerencia />
        </div>
        <div className="card">
          <div className="card-head"><div className="card-title">Bono Jefe de Ventas</div><span className="badge blue">3 meses</span></div>
          <BonoCB />
        </div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
        <div className="card">
          <div className="card-head"><div className="card-title">Win rate, ciclo y ticket promedio</div></div>
          <ProductividadTable />
        </div>
        <div className="card">
          <div className="card-head"><div className="card-title">Mix de cartera por vendedor</div></div>
          <MixVendedor />
        </div>
      </div>
    </>
...
// ============================================================
// RANKING TABLE — datos reales desde VENDEDORES
// ============================================================
function RankingTable() {
  const activos = D3.VENDEDORES.filter(v => v.activo);
  const presupMes = D3.KPI_MES ? D3.KPI_MES.presupuesto : 0;
    ...
    <table className="tbl">
      <thead>
        <tr>
          ...
          <th>Vendedor</th>
          <th>Cargo</th>
          <th className="num">Meta M$</th>
          <th className="num">Cotizado MTD</th>
          <th className="num">NV MTD</th>
          <th className="num">Facturado MTD</th>
          <th>Cumpl. MTD</th>
          <th className="num">Comisión proy.</th>
        </tr>
      </thead>
      <tbody>
        {activos.map((v, i) => {
      ...
      <div className="vendedor-card vacante">
        <div className="vendedor-head">
          ...
          <div className="v-info">
            ...
            <div className="v-cargo">{v.cargo}</div>
          </div>
        </div>
...
// ============================================================
// COSTOS TABLE — estático por ahora
// ============================================================
...
function ProductividadTable() {
  // Calculado dinámicamente desde VENDEDORES
    ...
    <table className="tbl">
      <thead>
        <tr>
          ...
          <th className="num">NV MTD</th>
          <th className="num">Facturado MTD</th>
          <th className="num">Conv. cot→NV</th>
          <th className="num">Conv. NV→Fact</th>
        </tr>
      </thead>
      <tbody>
        {activos.map((v,i) => {
...
function MixVendedor() {
  ...
  return (
    <div>
      ...
      {activos.length === 0 && (
        <div className="text-3 fs-11 text-center mt-3">Cargando datos del equipo...</div>
      )}
    </div>
  );
}
